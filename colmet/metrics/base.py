...
class BaseType(object):
    ...
    def __eq__(self,other):
        return self.struct_code == other.struct_code and self.length == other.length
...
class MetaCountersType(type):
    '''
    This type handle the counters/headers registration for the metric. For 
    each counter/header defined in the list _counters/_headers, it create 
    a corresponding property and register their specifities for the 
    class usage.
    '''
    ...
    _counter_representations = {
        'bytes': lambda value: "%s (bytes)"
            % MetaCountersType._normalize( value, 1024, 10000,  ['', 'K', 'M', 'G', 'T'] ),
        'kbytes': lambda value: "%s (bytes)" 
            % MetaCountersType._normalize(value, 1024, 10000, ['K', 'M', 'G', 'T']),
        "mbytes": lambda value: "%s (bytes)"
        ...
        "nsec": lambda value: "%s (seconds)" 
            % MetaCountersType._normalize(value, 1000, 1000, ['n', 'u', 'm', '']),
        "count": lambda value: "%s (count)" 
            % MetaCountersType._normalize(value, 1000, 10000, ['', 'K', 'M', 'G', 'T']),
        "mbytes-usec": lambda value: "%s (bytes*seconds)"
            % MetaCountersType._normalize(float(value*1024*1024/1000/1000), 1024, 10000, [' ', 'K', 'M', 'G', 'T']),
        "n/a": lambda value: "%s"
            % str(value)
    }
    ...
    _counter_accumulation_functions = {
        'add'  : lambda x,y,coeff: y if x == None else x if y == None else x + coeff * y,
        'none' : lambda x,y,coeff: y if x == None else x,
        'min'  : lambda x,y,coeff: y if x == None else x if y == None else x if x < y else y,
        'max'  : lambda x,y,coeff: y if x == None else x if y == None else x if x >= y else y,
    }
    ...
    def __new__(cls,name,bases,attrs):
        ...
        if len(bases) == 1:
            parent_cls = bases[0]
            for key in ['_header_definitions','_counter_definitions','_fmt','_counter_represensations','_counter_accumulation_functions']:
    ...
    def _set_header_property(cls,name):
        '''
        Internal function to set the property setter/getter for a given header value
        '''
    ...
    def _set_counter_property(cls,name):
        '''
        Internal function to set the property setter/getter for a given counter value
        '''
        fget = lambda t: t._get_counter(name)
        fset = lambda t,value: t._set_counter(name,value)
        setattr(cls,name,property(fget,fset))

    def registerCounter(cls,c_name,c_type,c_representation,c_accumulate_function,c_descr,c_index=None):
        '''
        Register a counter for the given class.

        This method must be used as a class method when defining a derived class of BaseCounters
        '''
        ...
        if c_index == None:
            if len(cls._counter_definitions)>0:
               c_index = 1 + max( [index for (_,(_,_,_,_,index,_)) in cls._counter_definitions.iteritems() ])
            else:
               c_index = 0
    ...
    def registerHeader(cls,h_name,h_type,h_representation,h_index=None):
        ...
        if h_index == None:
            if len(cls._header_definitions)>0:
               h_index = 1 + max( [index for (_,(_,_,index,_)) in cls._header_definitions.iteritems() ])
            else:
               h_index = 0
    ...
    def _updateStructFmt(cls):
        ...
        '''
        offset = 0
        for (h_name,(h_type,h_representation,h_index, _)) in sorted(cls._header_definitions.items(),key=lambda(k,v):v[2]):
            cls._header_definitions[h_name] = (h_type,h_representation,h_index, offset)
            offset += h_type.length
        
        for (c_name,(c_type,c_representation,c_acc_fn, c_descr, c_index, _)) in sorted(cls._counter_definitions.items(),key=lambda(k,v):v[4]):
        ...
        header_fmt_list = [ 
            ...
            for (h_name,(h_type,_,_,_)) in sorted(cls._header_definitions.items(),key=lambda(k,v):v[2])
        ]
        counter_fmt_list = [ 
            (c_name,c_type.struct_code) 
            for (c_name,(c_type,_,_,_,_,_)) in sorted(cls._counter_definitions.items(),key=lambda(k,v):v[4])
        ]
...
class BaseCounters(object):
    ...
    def accumulate(self, other_stats, destination, coeff=1):
        ...
        for counters in [ self,other_stats, destination ]:
            if counters._packed: counters.unpack()
        ...
        for (name, (_, _, acc_fn, _ ,_,_)) in self._counter_definitions.iteritems():
            d_counters[name] =  self._counter_accumulation_functions[acc_fn](
                    s_counters[name],
                    o_counters[name],
                    coeff
            )
