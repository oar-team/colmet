...
from colmet.exceptions import JobNeedToBeDefinedError, \
    FileAlreadyOpenWithDifferentModeError
...
class HDF5InputBackend(InputBaseBackend):
    ...
    def __init__(self, options):
        ...
        if self.job_id_list is None or len(self.job_id_list) == 0:
...
class JobFile(object):
    ...
    def __init__(self, options, job_id, filemode):
        ...
        if self.job_filemode == 'r':
            LOG.debug("Reading counters in hdf5 format for job %s"
                      % self.job_id)
            ...
            if (hasattr(options, 'hdf5_input_filepath')
                    and options.hdf5_input_filepath is not None):
    ...
    def _init_job_file_if_needed(self, metric_backend=None):
        ...
        if table_path not in self.job_file:
            ...
            self.job_metric_hdf5_class = \
                get_hdf5_class(self.job_metric_backend)
